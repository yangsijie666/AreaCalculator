...
func printWorkOrderInPdfFormatExecutor(report *core.Report) {
    ...
    if cTotalNumber != nil {
        cTotalArea.SetElement(gopdf.NewTextCell(table.GetColWidth(1, 0), 15, lineSpace, report).SetFont(fNormal).SetBorder(border).SetContent("总面积累积和：" + TotalArea))
